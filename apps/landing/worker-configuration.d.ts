...
/**
 * @deprecated Use AiMultimodalEmbeddingsOutput instead.
 * Backwards-compatible alias for old type name.
 */
export type AiIMultimodalEmbeddingsOutput = AiMultimodalEmbeddingsOutput;
...
/**
 * @deprecated Use AiTextGenerationToolOutput instead.
 * Legacy tool output format. Has 'name' and 'arguments' properties.
 */
 ...
 * Discriminator: has 'id' and 'type' properties (legacy has 'name' and 'arguments').
 * Use type guards to distinguish: `'id' in tool` for new shape, `'name' in tool` for legacy.
 */
...
/**
 * Tool calls can be either legacy or new format.
 * To safely handle: Check if element has 'id' property for new shape,
 * or 'name' property for legacy shape.
 *
 * Example:
 * ```ts
 * const toolCall = output.tool_calls?.[0];
 * if (toolCall && 'id' in toolCall) {
 *   // New format: AiTextGenerationToolOutput
 *   const name = toolCall.function.name;
 *   const args = JSON.parse(toolCall.function.arguments);
 * } else if (toolCall && 'name' in toolCall) {
 *   // Legacy format: AiTextGenerationToolLegacyOutput
